...
/*Clamav-milter는 sendmail 메일 서버의 필터다. 그것은 clamd에 내장된 메일 스캐닝 엔진을 사용한다.

Clamav-milter는 로드 밸런싱 및 내결함성 기술을 사용하여 둘 이상의 Clamd 서버에 연결하고 
서로 다른 컴퓨터 사이의 부하에도 원활하게 핫 스왑하고 서버가 다운되더라도 바이러스를 계속 검사할 수 있다.
*/
